...
PanelWindow {
    ...
    visible: bar.fullBarVisible
    anchors { top: true; left: true; right: true }
    exclusionMode: bar.fullBarVisible ? ExclusionMode.Auto : ExclusionMode.Ignore
    implicitHeight: bar.fullBarVisible ? 54 : 0
    color: "transparent"
    ...
    // full bar toggle — hides bar and releases exclusion for full screen
    property bool fullBarVisible: true
    // bar sides toggle — leaves middle island
    ...
    IpcHandler {
        target: "bar"
        function toggle(): void { bar.fullBarVisible = !bar.fullBarVisible }
        function toggleSides(): void { bar.barsVisible = !bar.barsVisible }
    ...
    Item {
        ...
        anchors.fill: parent
        visible: bar.fullBarVisible
        opacity: bar.fullBarVisible ? 1 : 0
        ...
        Item {
            ...
            MouseArea {
                ...
                onClicked: (mouse) => {
                    if (mouse.button === Qt.RightButton) bar.dynamicIsland = !bar.dynamicIsland
                }
            }
